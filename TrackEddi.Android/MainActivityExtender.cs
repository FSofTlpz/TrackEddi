   ...
   partial class MainActivity {
      ...
      async void onCreateExtend(Bundle savedInstanceState, string[] permissions) {
         ...
         if (permissionsOK)
            ...
            //AndroidX.AppCompat.App.AlertDialog dlg = new AndroidX.AppCompat.App.AlertDialog.Builder(this).Create();
            //dlg.SetTitle("Sorry");
            //dlg.SetMessage("Der App fehlen die für ihre Ausführung notwendigen Rechte.");
            //dlg.SetButton(-3,              // Which button to set the message for, can be one of BUTTON_POSITIVE (-1), BUTTON_NEGATIVE (-2), or BUTTON_NEUTRAL (-3)
            //              "OK", 
            //              (c, ev) => {
            //   Finish();
            //});

            Android.App.AlertDialog dlg = new Android.App.AlertDialog.Builder(this).Create();
            dlg.SetTitle("Sorry");
            dlg.SetMessage("Der App fehlen die für ihre Ausführung notwendigen Rechte.");
            dlg.SetButton("OK",
                           (c, ev) => {
