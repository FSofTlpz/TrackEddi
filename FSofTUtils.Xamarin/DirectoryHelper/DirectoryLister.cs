...
namespace FSofTUtils.Xamarin.DirectoryHelper {
   ...
   class DirectoryLister {
      ...
      public List<DirItem> Content(string path = null,
         ...
         if (!string.IsNullOrEmpty(path) &&
            ...
            foreach (StorageHelper.StorageItem item in lst) {
               if (item.IsDirectory) {
                  if (withextdirinfo) // Infos über untergeordnete Dateien und Verzeichnisse
                     ActualContent.Add(buildItem4Dir(path, item.Name, isremovable, isemulated));
                  else
                     ActualContent.Add(buildItem4Dir(item.Name, item.LastModified, isremovable, isemulated));
               } else
            ...
            foreach (var root in Root) {     // ungültiger/unbekannter Pfad -> alle Roots auflisten
               if (withextdirinfo) {
                  DateTime dt = getExtDirectoryInfos(root.Path, out int subdirs, out int files);
      ...
      /// <summary>
      /// ACHTUNG: rel. zeitintensiv, da für die Infos ein Aufruf von <see cref="StorageHelper.StorageItemList(string)"/> erfolgt
      /// </summary>
      /// <param name="dirpath"></param>
      /// <param name="subdirs"></param>
      /// <param name="files"></param>
      /// <returns></returns>
      DateTime getExtDirectoryInfos(string dirpath, out int subdirs, out int files) {
         subdirs = 0;
         files = 0;
      ...
      DirItem buildItem4Dir(string path, string subdir, bool isremovable, bool isemulated) {
         DateTime dt = getExtDirectoryInfos(Path.Combine(path, subdir), out int subdirs, out int files);
      ...
      DirItem buildItem4Dir(string dirname, DateTime dt, bool isremovable, bool isemulated) {
         return new DirItem(dirname,
                            isremovable,
                            isemulated,
                            -1,
                            -1,
      ...
      /// <summary>
      /// alle Objekte entfernen, die nicht zur jeweilgen RegEx passen
      /// </summary>
      /// <param name="lst"></param>
      /// <param name="regexFileMatch"></param>
      /// <param name="regexDirMatch"></param>
      void removeUnmatchingItems(List<StorageHelper.StorageItem> lst, Regex regexFileMatch, Regex regexDirMatch) {
         for (int i = lst.Count - 1; i >= 0; i--) {
            if (lst[i].IsFile) {
               if (regexFileMatch != null)
                  if (!regexFileMatch.IsMatch(lst[i].Name))
                     lst.RemoveAt(i);
            } else {
               if (regexDirMatch != null)
                  if (!regexDirMatch.IsMatch(lst[i].Name))
                     lst.RemoveAt(i);
            }
         }
      }
